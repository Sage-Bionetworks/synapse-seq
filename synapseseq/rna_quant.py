...
def runSailfish(prefix,wd,R1,R2,commandsFile=cfHandle):
	...
	if config['workflow']['paired'] is True:	
		cmd = ' '.join(['sailfish quant -i', config['sailfish']['index'] '-1', R1, '-2', R2,  '-o', os.path.join(wd,prefix), config['sailfish']['options']])
	else:
		cmd = ' '.join(['sailfish quant -i', config['sailfish']['index'] '-1', R1,  '-o', os.path.join(wd,prefix), config['sailfish']['options']])
