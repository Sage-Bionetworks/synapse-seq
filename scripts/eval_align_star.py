...
parser.add_argument('--params', required=False, help='Synapse ID of the parameter file for this job. Required if not specified in config file.')
parser.add_argument('--config', dest='conf', required=False, help='Path to local config file. Required.')
parser.add_argument('--ref', dest='ref', required=False, help='Synapse ID of reference genome used for alignment. Required if not specified in config file.', default=None)
parser.add_argument('--output', dest='out', required=False, help='Synapse ID of project or folder to contain the output data. Required if not specified in config file.')
parser.add_argument('--threads', dest='threads', required=False, help='Number cpus to use per job.', default=4)
parser.add_argument('--toFollow', dest='follow', required=False, help='Synapse ID of evaluation to run on the data following this one.', default=None)
...
subprocess.call(' '.join(['sudo chown ubuntu:ubuntu', config['localWDPath']]), shell = True)
...
evalName = 'ALIGNstar'
wd = os.path.join(config['localWDPath'], evalName)
...
## Get index
index = os.path.join(config['headNFSPath'], config['GRCh38_star_RL100'])
...
followingSubmission = syn.submit(entity=aligneEntity, evaluation = args.follow,  name = alignEntity.name, teamName = submission.teamName)
print 'Submitted %s to %s' % (aligneEntity.name, args.follow)

cmd = ' '.join([codePath, '--submission', followingSubmission.id, additionalParams])
print '%s' % cmd
subprocess.call(cmd, shell = True)
